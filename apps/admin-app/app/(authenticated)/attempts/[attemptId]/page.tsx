...
export default function AttemptDetailsPage() {
  ...
  const hasAttemptNo = attemptNo !== "?";
  const attemptLabel = hasAttemptNo ? `Attempt #${attemptNo}` : "Attempt";
  const isCertificateReferrer = referrer?.startsWith("/certificates/") ?? false;

  // Fallback back navigation
  const backHref = referrer ?? (participantId ? `/participants/${participantId}` : "/certificates");

  const { data: participant } = useParticipant(participantId ?? "");
  const { data: review, isLoading, isError } = useAttemptReview(participantId ?? "", attemptId ?? "");
  ...
  if (isError || !review || !participantId) {
    return (
    ...
    <div className="p-8 space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-sm" style={{ color: "var(--text-muted)" }}>
          {isCertificateReferrer ? (
            <>
              <Link href="/certificates" className="hover:underline">Certificates</Link>
              <span>/</span>
              <Link href={backHref} className="hover:underline">Certificate Details</Link>
              <span>/</span>
            ...
            <>
              ...
              <span>/</span>
              <Link href={`/participants/${participantId}`} className="hover:underline">
                {participant?.name ?? "..."}
              </Link>
            </>
          )}
          <span>/</span>
          <span style={{ color: "var(--text)", fontWeight: 600 }}>{attemptLabel} Review</span>
        </div>
      </div>
        ...
        {review.questions.map((q) => (
          ...
          >
            ...
            >
              ...
              <div className="shrink-0 flex items-center gap-2">
                ...
                <span
                  ...
                  style={{
                    background: q.isCorrect ? "rgba(34,197,94,0.10)" : "rgba(220,38,38,0.10)",
                    color: q.isCorrect ? passColor : failColor,
                  }}
                >
                  {q.isCorrect ? <IconCheck size={11} /> : <IconX size={11} />}
            ...
            <div className="px-4 py-3 space-y-3">
              {/* User answer */}
              <div>
                <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>YOUR ANSWER</p>
                <p
                  className="text-sm px-3 py-2 rounded-lg"
                  style={{
                    background: q.isCorrect ? "rgba(34,197,94,0.08)" : "rgba(220,38,38,0.08)",
                    color: q.isCorrect ? passColor : failColor,
                  }}
                >
                  {q.yourAnswer || "—"}
              ...
              {!q.isCorrect && q.correctAnswer && (
                <div>
                  <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>CORRECT ANSWER</p>
                  <p
                    className="text-sm px-3 py-2 rounded-lg"
                    style={{
              ...
              {q.explanation && (
                <div>
                  <p className="text-xs font-semibold mb-2" style={{ color: "var(--text-muted)" }}>EXPLANATION</p>
