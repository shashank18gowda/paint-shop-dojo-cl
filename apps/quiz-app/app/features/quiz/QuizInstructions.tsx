...
import { useQuizEligibility } from "../../lib/hooks/useQuiz";
import { PlayIcon, QuizCheckIcon, Spinner } from "../../components/icons";
import { PageShell, PageHeader } from "../../components/layout/PageShell";

export default function QuizInstructions() {
  const router = useRouter();
  const ui = useTranslation("quiz");
  const { data: eligibility, isLoading } = useQuizEligibility();

  void useFlowStore((s) => s.lang);
      ...
      <div className="flex flex-1 flex-col items-center gap-8 max-w-2xl mx-auto w-full">
        <div className="flex flex-col items-center gap-3 text-center">
          <div
            ...
            style={{ background: "rgba(235,10,30,0.12)" }}
          >
            <QuizCheckIcon size={28} />
          </div>
          <div>
        ...
        <div className="w-full grid grid-cols-2 gap-3">
          <StatCard icon="📋" value={ui.questionCount} />
          <StatCard icon="⏱" value={ui.totalDuration} />
        </div>
...
function StatCard({ icon, value }: { icon: string; value: string }) {
  return (
    <div
      className="flex flex-col items-center gap-1 rounded-2xl py-4 px-3 text-center"
      style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}
    >
